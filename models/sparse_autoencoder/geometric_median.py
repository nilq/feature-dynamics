...
def geometric_median(
    ...
    eps: float = 1e-6,
    max_iter: int = 100,
    fractional_improvement_tolerance: float = 1e-20,
) -> torch.Tensor:
    ...
    objective_value = geometric_median_objective(
        median=median, points=points, weights=weights
    )
    ...
    for _ in tqdm.tqdm(range(max_iter), desc="Computing geometric median"):
        ...
        new_objective_value = geometric_median_objective(
            median=updated_median, points=points, weights=weights
        )

        if (
            abs(objective_value - new_objective_value)
            <= fractional_improvement_tolerance * objective_value
        ):
            break
