...
def select_activation_examples(df: pd.DataFrame, feature_number: int) -> pd.DataFrame:
    ...
    other_intervals_indices = feature_data[feature_data['quantized_activation'] < 9].index

    top_examples = feature_data.loc[top_interval_indices].nlargest(10, 'activation')

    other_examples = feature_data.loc[other_intervals_indices].groupby('quantized_activation').apply(lambda x: x.nlargest(2, 'activation')).reset_index(drop=True)
    random_examples = feature_data.sample(5)

    top_tokens = top_examples['token'].unique()
    different_context_examples = feature_data[feature_data['token'].isin(top_tokens) & ~feature_data.index.isin(top_examples.index)].drop_duplicates('context').head(10)

    selected_examples = pd.concat([top_examples, other_examples, random_examples, different_context_examples])
